...
exports.HttpSecuritySchemes = ({ securities, title, className }) => {
    ...
    return (React.createElement("div", { className: classnames_1.default('HttpSecuritySchemes', className) },
        ...
        React.createElement("div", { className: "mt-6 border rounded TreeList dark:border-darken" }, lodash_1.flatten(securities).map((security, index) => {
            const securityClassName = classnames_1.default('TreeListItem', {
                'TreeListItem--striped': index % 2 !== 0,
            });
            return React.createElement(SecurityScheme, { key: index, className: securityClassName, security: security });
        }))));
};
exports.HttpSecuritySchemes.displayName = 'HttpSecuritySchemes';
const SecurityScheme = ({ security, className }) => {
    return (React.createElement("div", { className: classnames_1.default(className, 'HttpSecuritySchemes__SecurityScheme p-4 flex items-start'), style: { alignItems: 'start' } },
        React.createElement("div", { style: { minWidth: '60px' } },
            React.createElement("div", null, security.key),
            React.createElement("div", { className: `text-sm text-${http_1.HttpSecuritySchemeColors[security.type]}-7 dark:text-${http_1.HttpSecuritySchemeColors[security.type]}-6` }, security.type)),
